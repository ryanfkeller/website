...
describe("Flag Issues With Deleted Labels Workflow Tests", () => {
  ...
  const checkAgenda = async ({
    ...
    const agendaIssue = await gh.getIssue(STATIC_ISSUE_NUMS.AGENDA);
    if (!agendaIssue) {
  ...
  describe("nominal tests", () => {
    it("one in-use label deleted", async () => {
      console.info("Running test: one in-use label deleted (duration ~20s)");
